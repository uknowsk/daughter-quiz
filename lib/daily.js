...
//          → (3) 정적 문제은행 폴백(무료, 기본 동작) → (4) AI 실시간 생성
//          ※ (4)는 ALLOW_LIVE_AI_FALLBACK=true 를 로아빠가 직접 켰을 때만, "문제은행이 바닥난 비상시"에만 동작(기본 꺼짐).
...
import { generateQuestions, hasApiKey, liveFallbackEnabled } from "./generate";
import {
  getProgress,
  getDailySet,
  saveDailySet,
...
export async function getOrCreateDailySet(student, subject, { force = false } = {}) {
  ...
  // 2) 은행에 없으면(예: "하" 난이도, 아직 못 만든 단원 등) → 정적 폴백(무료)이 우선.
  //    실시간 AI 생성은 로아빠가 명시적으로 허락(ALLOW_LIVE_AI_FALLBACK=true)했을 때만, 그것도 "비상용"으로만 호출.
  if (!set && liveFallbackEnabled() && hasApiKey()) {
