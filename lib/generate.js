...
// 안전장치: API 키가 있어도, 이 값이 명시적으로 "true"가 아니면 실시간 생성은 절대 호출하지 않음.
// 로아빠 지시(2026-07-31): "문제은행이 바닥났을 때 급할 때만" 쓰는 비상용 스위치. 기본은 항상 꺼짐(무료).
// 켜고 싶으면 Vercel 환경변수에 ALLOW_LIVE_AI_FALLBACK=true 를 직접 추가해야 함.
export function liveFallbackEnabled() {
  return process.env.ALLOW_LIVE_AI_FALLBACK === "true";
}
