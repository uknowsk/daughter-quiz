...
function buildPrompt({ grade, subject, unit, level, avoid }) {
...
export async function generateQuestions({ grade, subject, unit, level, avoid }) {
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        model: MODEL,
        max_tokens: 3000,
        messages: [{ role: "user", content: buildPrompt({ grade, subject, unit, level, avoid }) }],
      }),
    });

    if (!res.ok) {
      console.error("문제 생성 API 오류:", res.status, await res.text());
      return null;
    }
