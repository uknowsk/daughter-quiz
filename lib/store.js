...
export async function addWrongNotes(userId, subject, level, wrongItems, conceptTitle) {
  const all = await read("wrongnotes", {});
  ...
  for (const w of wrongItems) {
    ...
    const entry = {
      ...
      answer: w.answer,
      image: w.image || null,
      explain: w.explain || null,      // 문제에 해설이 있으면 사용
      conceptTitle: conceptTitle || null, // 없으면 관련 개념 제목으로 안내
      addedAt: Date.now(),
    };
...
// ---- AI가 생성한 그날의 문제 세트 캐시 (사용자·과목·날짜별 1회 생성 후 재사용) ----
export async function getDailySet(userId, subject, dateStr) {
  const all = await read("dailyset", {});
  return all[`${dateStr}|${userId}|${subject}`] || null;
}

export async function saveDailySet(userId, subject, dateStr, set) {
  const all = await read("dailyset", {});
  const keep = {};
  const cutoff = `${dateStr}`;
  // 오래된 캐시는 정리(오늘·미래 것만 유지)
  for (const [k, v] of Object.entries(all)) {
    if (k.split("|")[0] >= cutoff) keep[k] = v;
  }
  keep[`${dateStr}|${userId}|${subject}`] = set;
  await write("dailyset", keep);
}

// ---- 중복 방지용: 과거에 낸 문제 텍스트 기록 (사용자·과목별 최근 N개) ----
export async function getAskedHistory(userId, subject) {
  const all = await read("asked", {});
  return all[`${userId}|${subject}`] || [];
}

export async function addAskedHistory(userId, subject, questionTexts) {
  const all = await read("asked", {});
  const key = `${userId}|${subject}`;
  const merged = [...(all[key] || []), ...questionTexts];
  // 최근 120개만 유지
  all[key] = merged.slice(-120);
  await write("asked", all);
}
